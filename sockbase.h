...
struct CSockAddrIn
{
	...
	// ポートとIPv4アドレスによる初期化
	CSockAddrIn(USHORT port = 0, ADDRESS_FAMILY family = AF_INET)
	{
		sin_family = family;
		sin_port = htons(port);
	...
	CSockAddrIn(const char* addr_v4, USHORT port)
	{
		sin_family = AF_INET;
		sin_port = htons(port);
		
		((SOCKADDR_IN*)*this)->sin_addr.s_addr = inet_addr(addr_v4);
	}
...
public:
	CSocketBaseT(SOCKET hSocket = INVALID_SOCKET)
	{
		m_hSocket = hSocket;
	}
