...
#define MAX_WAITCOUNT (INFINITE / 2 + 1)
...
public:
	...
	{
		m_hIocp = NULL;
		m_dwMinKeepAlive = MAX_WAITCOUNT;
	...
	~CIocpThreadPool()
	{
		...
		if (!m_pThreads && m_hIocp)
			::CloseHandle(m_hIocp);	// スレッド停止中にIOCPハンドル解放
	}
	...
	BOOL Start(DWORD nKeepAlive = MIN_KEEPALIVE, BOOL bSync = TRUE)
	{
		...
		// タイムアウト時間設定(MAX_WAITCOUNTは終了時の予約値)
		m_dwMinKeepAlive = (nKeepAlive < MAX_WAITCOUNT ? nKeepAlive : INFINITE);
	...
	{
		if (!m_hIocp)
			return FALSE;	// IOCP未作成
		
		// タイムアウト値の更新判定によって再入防止
		if (::InterlockedExchange(&m_dwMinKeepAlive, MAX_WAITCOUNT) != MAX_WAITCOUNT)
			::CloseHandle(m_hIocp);	// IOCPハンドル解放によりスレッド終了
		
		if (!bSync || ::InterlockedExchange(&m_nThreadCnt, 0) <= 0)	// スレッド終了判定
