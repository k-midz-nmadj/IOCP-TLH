...
// IOCP対応サーバークラス(TYPE: CIocpSocket派生クラス, SYNC: 同期/非同期)
...
protected:
	...
	{
		CSocketFactory* pFactry = reinterpret_cast<CSocketFactory*>(lpCPEntry->Internal);
		
		if (lpCPEntry->dwNumberOfBytesTransferred <= 0xFFFF)	// ポート指定: サーバソケット作成
			pFactry->AddListener<TYPE>(
				(USHORT)lpCPEntry->dwNumberOfBytesTransferred, 
				(int)lpCPEntry->lpCompletionKey);
		else	// IPアドレス指定: クライアントソケット作成
			pFactry->AddConnection<TYPE>(
				CSockAddrIn((USHORT)lpCPEntry->lpCompletionKey, lpCPEntry->dwNumberOfBytesTransferred));
	}
public:
	...
	BOOL AddListener(USHORT nPort, int nConnections = SOMAXCONN)	// サーバソケット作成
	{
		return (CreateIocp() && PostEvent(nConnections, nPort));
	}
	BOOL AddConnection(const char* addr_v4, USHORT nPort)	// クライアントソケット作成
	{
		return (CreateIocp() && PostEvent(nPort, inet_addr(addr_v4)));
	}
	...
	{
		if (IsRunning() || !CreateIocp(NULL, nThreadCnt))	// IOCP作成
			return FALSE;
		
		// スレッドプール実行(実行直後、ユーザイベントによってサーバ起動)
		return (AddListener(nPort, nConnections) && Start(nKeepAlive, SYNC));
	}
};
