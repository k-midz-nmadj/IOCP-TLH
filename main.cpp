...
class CIocpSocketImpl : public CIocpSocket
{
	...
	{
		m_bRemote = (ULONG(*pAddrRemote) != 0x0100007f);	// リモート判定
...
int main()
{
	...
	CIocpServer<CIocpSocketImpl> svr;

	if (!svr.AddListener(PORT_NUM))	// ポートオープン
		return -1;
	
	if (!svr.AddConnection("127.0.0.1", PORT_NUM))	// ローカル接続
		return -1;
