...
public:
	...
	DWORD CreateThread(LPTHREAD_START_ROUTINE fpThreadProc, LPVOID pParam, BOOL bSync = FALSE)
	{
		if (m_hThread)
			return -1;	// スレッド作成済み
		...
		if (bSync)	// 同期有効
		{
			...
			return (m_hThread ? fpThreadProc(pParam) : -1);	// 関数終了までブロック
		}
#ifdef _INC_PROCESS
		m_hThread = (HANDLE)::_beginthreadex(NULL, 0, (_beginthreadex_proc_type)fpThreadProc, pParam, 
														CREATE_SUSPENDED, (UINT*)&m_dwThreadID);
#else
		m_hThread = ::CreateThread(NULL, 0, fpThreadProc, pParam, CREATE_SUSPENDED, &m_dwThreadID);
#endif
